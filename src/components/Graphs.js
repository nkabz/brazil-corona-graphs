...
function Graphs() {
    ...
    const chartGraph = mapData ?
        <VictoryChart
            ...
            containerComponent={
                <VictoryVoronoiContainer
                    labels={({datum}) => `${moment(datum.x).format('MMM-YYYY').toString()}\nConfirmados: ${datum.y}`}
                    labelComponent={
                        <VictoryTooltip
                            cornerRadius={5}
                            flyoutStyle={{
        ...
        >
            <VictoryAxis
                tickCount={tickCount}
                tickLabelComponent={<VictoryLabel angle={-35} dy={10} />}
            />
            <VictoryAxis
                dependentAxis
            />
            <VictoryLine
                data={mapData}
                style={{data: { stroke: "#c43a31" }, labels: {fill: "#c43a31", fontSize: 16, padding: 5}}}
            />
        </VictoryChart>
        : null
